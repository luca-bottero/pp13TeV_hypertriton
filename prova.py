...
for score in score_from_efficiency_array(train_test_data[3],y_pred_test,np.arange(min_eff,max_eff,step)):
    ...
    utils.scatter_with_hist(sel['m'],sel['mppi'],[34,2.96,3.04],[34,np.min(sel['mppi']),np.max(sel['mppi'])],
                            x_label='Hypertriton mass [GeV/$c^2$]',
                            y_label='$p - \pi$ mass [GeV/$c^2$]', eff = i)
    i += step


utils.scatter_with_hist(data.get_data_frame().query('model_output > 4')['m'],
                            np.sqrt(np.array(data.get_data_frame().query('model_output > 4')['mppi'])),
                            [34,2.96,3.04],[34,1.05,1.2],
                            x_label='Hypertriton mass [GeV/$c^2$]',y_label='$p - \pi$ mass [GeV/$c^2$]')
